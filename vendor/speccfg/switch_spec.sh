...
installFunc()
{
  ...
  then
    ...
    if [ "$mode" != "compiling" ] && [ "$(getprop ro.regionalization.support)" == "true" ]
    then
      echo "Regionalization environment has been enabled!"
      echo "Can't change system partition and then no need link!"
    else
...
uninstallFunc()
{
  ...
  then
    ...
    then
      echo "Regionalization environment has been enabled!"
      echo "Can't change system partition and then no need remove!"
    else
      rm $srcPath
    fi
...
then
  ...
  then
    SwitchData1="$DestFolder/data/data/com.qualcomm.qti.loadcarrier"
    SwitchData2="$DestFolder/data/data/com.qualcomm.qti.accesscache"
    ...
    then
      echo "packStorage=/system/vendor" > "$RegionalizationEnvSpecPath/spec"
    ...
    then
      echo "import /system/vendor/Default/system/vendor/vendor.prop" > "$RegionalizationEnvSpecPath/vendor_ro.prop"
    fi
    ...
    then
      echo "import /system/vendor/Default/system/vendor/vendor.prop" > "$RegionalizationEnvSpecPath/vendor_persist.prop"
    fi
